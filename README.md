...
The solver navigates a massive search spaces that can get as large as **~10²² possible configurations** and consistently finds solutions to hard-difficulty NYT Pips puzzles in **under 1 second** (typically ~900ms).

## How It Works

1. **Problem Representation**: Models the puzzle as a CSP where each square is a variable that must be assigned a domino half
2. **Constraint Propagation**: Uses GAC to eliminate impossible domino placements before search any branch
